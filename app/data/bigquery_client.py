...
class BigQueryClient:
    ...
    def __init__(self):
        ...
        self.dataset = os.getenv("BIGQUERY_DATASET", "profit_scout")

        if BigQueryClient._client_instance is None:
            # If project_id is not set, it might auto-detect in GCP environment
            if self.project_id:
                BigQueryClient._client_instance = bigquery.Client(project=self.project_id)
            else:
                BigQueryClient._client_instance = bigquery.Client()
    ...
    def _get_table_id(self, table_name: str) -> str:
        ...
        if "." in table_name:
            return table_name
        # Fallback to project_id from client if env var wasn't set
        project = self.project_id or self.client.project
        return f"{project}.{self.dataset}.{table_name}"
    ...
    async def get_winners_dashboard(
        ...
        table_name = os.getenv("WINNERS_DASHBOARD_TABLE", "options_analysis_signals")
        table_id = self._get_table_id(table_name)
